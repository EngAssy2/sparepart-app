...
import { PackageMinus, Plus, X, Search, AlertTriangle } from 'lucide-react';
...
export default function StockOutPage() {
    ...
    const handleSubmit = (e) => {
        ...
        for (let i = 0; i < validItems.length; i++) {
            ...
            if (it.Quantity > matchedPart.Quantity) {
                setError(`Item ${i + 1} (${it.Part_Number}): Quantity (${it.Quantity}) exceeds available stock (${matchedPart.Quantity}).`);
                return;
            }
        }
        setShowConfirm(true);
    ...
    const executeSubmit = async () => {
        ...
        try {
            ...
            const res = await client.post('/transactions/stock-out', { items: validItemsPayload });
            setSuccess(`Stock Out recorded! Processed ${res.data.totalProcessed} item(s).`);
            ...
            setError(err.response?.data?.error || err.response?.data?.message || 'Failed to record Stock Out.');
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <AppShell title="Stock Out">
            <div className="page-header">
                <div>
                    ...
                    <div className="page-subtitle">Remove inventory quantities from spare parts</div>
                </div>
            </div>
                ...
                <div className="card">
                    <form onSubmit={handleSubmit} onKeyDown={(e) => { if (e.key === 'Enter' && e.target.tagName !== 'BUTTON') e.preventDefault(); }}>
                        ...
                        <div className="public-stockout-items-section" style={{ background: 'transparent', padding: '0 0 16px 0', border: 'none' }}>
                            <div className="public-stockout-items-header">Multiple Part Withdrawal</div>
                                ...
                                return (
                                    <div key={idx} className="public-stockout-item-row" style={{ alignItems: 'flex-start' }}>
                                        ...
                                        <div style={{ width: 90 }}>
                                            ...
                                            />
                                            {overStock && (
                                                <div className="field-hint field-hint-error" style={{marginTop: 4}}>Max: {stockQty}</div>
                                            )}
                                        </div>
                                        <div style={{ flex: 1.5 }}>
                                            <select
                        ...
                        <div style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end' }}>
                            <button className="btn btn-danger btn-lg" type="submit" disabled={submitting}>
                                {submitting
                                    ...
                                    : <><PackageMinus size={16} /> Finalize Stock Out</>
                                }
                            </button>
                        </div>
                    </form>
            ...
            <div className="modal-overlay" style={{ zIndex: 9999 }} onClick={(e) => { if (e.target === e.currentTarget) setShowConfirm(false); }}>
                <div className="modal" style={{ maxWidth: 420 }}>
                    ...
                    <div style={{ padding: '20px 0', lineHeight: 1.6, fontSize: 15 }}>
                        Are you sure you want to withdraw <strong>{items.filter(i => i.Part_Number && i.Quantity > 0).length} item(s)</strong> from stock?
                    </div>
                    <div className="modal-footer">
                        <button className="btn btn-secondary" onClick={() => setShowConfirm(false)}>Cancel</button>
                        <button className="btn btn-danger" onClick={executeSubmit} disabled={submitting}>
